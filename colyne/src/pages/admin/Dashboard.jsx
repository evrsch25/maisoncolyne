  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          ...
          >
            {loading ? (
              ...
              <p className="text-sm text-gray-400 italic py-4 text-center">Aucun message reçu pour le moment.</p>
            ) : (
              <div className="divide-y divide-gray-50">
                {recentContacts.map((msg) => {
                  const s = statusConfig[msg.status] || statusConfig.nouveau;
                  return (
                    <Link
                      key={msg._id}
                      to="/admin/messages"
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-gray-400 truncate">{msg.serviceType} · {msg.email}</p>
                      </div>
        ...
        {/* Accès rapides */}
        <SectionCard title="Accès rapides" icon={LayoutDashboard} delay={0.35}>
          <div className="space-y-1">
            <QuickLink to="/admin/prestations" icon={FileText} label="Prestations" sublabel="Gérer vos offres" color="bg-indigo-500" />
            <QuickLink to="/admin/blog" icon={BookOpen} label="Blog" sublabel="Rédiger un article" color="bg-emerald-500" />
            <QuickLink to="/admin/portfolio" icon={Camera} label="Portfolio" sublabel={`${photosTotal} photos · ${photosTitrees} titrées`} color="bg-violet-500" />
            <QuickLink to="/admin/media" icon={Image} label="Médias statiques" sublabel="Bandeaux, illustrations" color="bg-sky-500" />
            <QuickLink to="/admin/testimonials" icon={Star} label="Témoignages" sublabel={`${data.testimonials.length} avis clients`} color="bg-amber-500" />
            <QuickLink to="/admin/actualites" icon={Megaphone} label="Actualités" sublabel="Promotions, événements" color="bg-rose-500" />
            <QuickLink to="/admin/pages" icon={Settings} label="Configuration site" sublabel="Textes, contact, FAQ" color="bg-slate-500" />
            <QuickLink to="/admin/pages-legales" icon={Scale} label="Pages légales" sublabel="CGV, mentions légales…" color="bg-teal-500" />
          </div>
        </SectionCard>
      </div>
